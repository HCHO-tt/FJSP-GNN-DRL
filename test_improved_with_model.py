...
def main(model_path, data_path="1005", num_ins=10, num_average=10):
    SEED = 42
    setup_seed(SEED)
    
    # 创建实验管理器
    exp_manager = ExperimentManager(base_dir="experiment_results", 
                                   experiment_name=f"improved_heuristic_trained_{data_path}")
    exp_manager.log("=" * 60)
    exp_manager.log(f"改进模型测试 (启发式特征增强 + 训练模型)")
    exp_manager.log("=" * 60)
    ...
    exp_manager.log(f"  模型路径: {model_path}")
    ...
    exp_manager.log(f"模型加载成功: {model_path}")
...
if __name__ == '__main__':
    model_path = sys.argv[1] if len(sys.argv) > 1 else "./save/train_improved_20260513_093252/model_best.pt"
    data_path = sys.argv[2] if len(sys.argv) > 2 else "1005"
    num_ins = int(sys.argv[3]) if len(sys.argv) > 3 else 5
    num_average = int(sys.argv[4]) if len(sys.argv) > 4 else 3
    
    main(model_path, data_path, num_ins, num_average)
