...
def main():
    ...
    with open(log_path, 'w', buffering=1) as log_file:
        ...
        # 获取测试文件
        data_path = f"./data_test/{test_paras['data_path']}/"
        test_files = os.listdir(data_path)
        test_files.sort(key=lambda x: x[:-4])
        test_files = test_files[:test_paras["num_ins"]]
        
        log_message(log_file, f"测试数据: {data_path}")
        ...
        # 加载预训练模型
        mod_files = [f for f in os.listdir('./model/') if f.endswith('.pt')]
        if mod_files:
            model_file = mod_files[0]
            log_message(log_file, f"加载模型: {model_file}")
            model_CKPT = torch.load('./model/' + model_file, map_location=device)
        ...
        # 保存结果 - 需要将torch device转换为字符串
        env_paras_save = env_paras.copy()
        env_paras_save["device"] = str(env_paras_save["device"])
        model_paras_save = model_paras.copy()
        model_paras_save["device"] = str(model_paras_save["device"])
